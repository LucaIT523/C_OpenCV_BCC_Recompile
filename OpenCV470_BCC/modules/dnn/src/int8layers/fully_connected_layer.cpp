            ...
            for( size_t ofs = stripeStart; ofs < stripeEnd; )
            {
            ...
            #endif
                {
                    ...
                    for( ; i  <= nw - 4; i += 4, wptr += 4*wstep )
                    {
                        ...
                        {
                            v_int8x16 v = (v_int8x16)v_load_aligned((const schar*)(sptr + k));
                            vs0 = v_dotprod_expand_fast(v, (v_int8x16)v_load_aligned((const schar*)(wptr + k)), vs0);
                            vs1 = v_dotprod_expand_fast(v, (v_int8x16)v_load_aligned((const schar*)(wptr + wstep + k)), vs1);
                            vs2 = v_dotprod_expand_fast(v, (v_int8x16)v_load_aligned((const schar*)(wptr + wstep*2 + k)), vs2);
                            vs3 = v_dotprod_expand_fast(v, (v_int8x16)v_load_aligned((const schar*)(wptr + wstep*3 + k)), vs3);
                        }
