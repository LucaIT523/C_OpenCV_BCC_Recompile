                        ...
                        if( isPool2D && xstart > 0 && x0 + 15 < x1 && (x0 + 15) * stride_w - pad_l + kernel_w < inp_width )
                        {
                            ...
                            if( yend - ystart == kernel_h )
                            {
                                ...
                                if( stride_w == 1 )
                                    ...
                                    {
                                        int index = ofsptr[k];
                                        v_int8x16 v0 = (v_int8x16)v_load((const schar*)(srcData1 + index));
                                    ...
                                    for (int k = 0; k < kernel_w*kernel_h; k++)
                                    {
                                        int index = ofsptr[k];
                                        v_int8x16 v0, dummy;
                                        v_load_deinterleave((const uchar*)(srcData1 + index), (v_uint8x16 &)v0, (v_uint8x16 &)dummy);
                                        max_val0 = v_max(max_val0, v0);
                                    }
                                else
                                    for (int k = 0; k < kernel_w*kernel_h; k++)
                            ...
                            v_store((schar*)(dstData + x0), (v_int8x16)max_val0);
                            x0 += 15;
                        }
                        else
#else
                ...
                else if (poolingType == AVE || poolingType == SUM)
                {
                    for( ; x0 < x1; ++x0)
                    {
                        ...
                        if( isPool2D && xstart > 0 && x0 + 15 < x1 && (x0 + 15) * stride_w - pad_l + kernel_w < inp_width )
                        {
                            ...
                            v_store((schar*)(dstData + x0), (v_int8x16)(v_pack(v_pack(sum_val0, sum_val1), v_pack(sum_val2, sum_val3))));
                            x0 += 15;
                        }
                        else
#endif
